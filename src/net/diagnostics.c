...
static const char *safe_string(const char *value) { return value ? value : ""; }

static int checked_snprintf(char *buffer, size_t buffer_len, const char *format, ...) {
    va_list args;
    int written;
...
int mochad_diag_json_health(char *buffer, size_t buffer_len, const char *upstream_base,
    ...
    return checked_snprintf(
        ...
        MOCHAD_REDUX_VERSION, safe_string(upstream_base), runtime->uptime_seconds,
        json_bool(runtime->usb_connected), safe_string(runtime->controller),
        json_bool(runtime->endpoints_ready), json_bool(runtime->transfers_ready),
        runtime->clients_main + runtime->clients_xml + runtime->clients_openremote,
        runtime->usb_out_completed, runtime->usb_ack_received, runtime->usb_ack_timeout,
        runtime->usb_unexpected_one_byte, runtime->config->bind_address,
        runtime->config->server_port, json_bool(runtime->config->xml_enabled),
        runtime->config->xml_port, json_bool(runtime->config->openremote_enabled),
