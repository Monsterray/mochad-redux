...
/*
 * Not defined on every POSIX platform - notably absent on macOS, which uses
 * the SO_NOSIGPIPE socket option instead. Falling back to 0 keeps send()
 * calls valid; on such platforms SIGPIPE must be suppressed another way.
 */
#ifndef MSG_NOSIGNAL
