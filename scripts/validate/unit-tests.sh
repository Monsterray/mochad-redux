...
"$CC" $CFLAGS tests/test_socket_io.c src/net/socket_io.c -o "$BUILD_DIR/test_socket_io" $LDFLAGS
"$BUILD_DIR/test_socket_io"

echo
echo "+ config"
# shellcheck disable=SC2086
"$CC" $CFLAGS tests/test_config.c src/config/config.c -o "$BUILD_DIR/test_config" $LDFLAGS
"$BUILD_DIR/test_config"

echo
echo "+ mochad_event"
# shellcheck disable=SC2086
"$CC" $CFLAGS tests/test_mochad_event.c src/x10/mochad_event.c \
...
echo "+ x10_write"
# shellcheck disable=SC2086
"$CC" $CFLAGS tests/test_x10_write.c src/usb/x10_write.c \
...
echo "+ encode_rf"
# shellcheck disable=SC2086
"$CC" $CFLAGS tests/test_encode_rf.c src/x10/encode.c \
...
echo "+ diagnostics"
# shellcheck disable=SC2086
"$CC" $CFLAGS tests/test_diagnostics.c src/net/diagnostics.c src/config/config.c \
...
# shellcheck disable=SC2086
"$CC" $CFLAGS tests/test_usb_endpoint_selection.c src/usb/usb_endpoint_selection.c \
