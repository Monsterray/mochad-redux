        ...
        char *buffer, size_t buffer_len)
{
    ...
    switch (event->detail) {
        case MOCHAD_EVENT_DETAIL_HOUSE_UNIT:
            if (event->has_extended_data) {
                return format_checked(buffer, buffer_len,
                        ...
                        direction, transport, event->house, event->unit,
                        event->function, event->extended_data,
                        event->extended_command);
            }
        ...
        case MOCHAD_EVENT_DETAIL_HOUSE:
            if (event->has_level) {
                return format_checked(buffer, buffer_len,
                        "%s %s House: %c Func: %s(%d)",
                        direction, transport, event->house, event->function,
                        event->level);
            }
            return format_checked(buffer, buffer_len,
                    "%s %s House: %c Func: %s",
        ...
        case MOCHAD_EVENT_DETAIL_RFSEC_SHORT:
            return format_checked(buffer, buffer_len,
                    "%s %s Addr: 0x%02X Func: %s",
                    direction, transport, event->security_short_addr,
                    event->function);

        case MOCHAD_EVENT_DETAIL_RFSEC_LONG:
            return format_checked(buffer, buffer_len,
                    "%s %s Addr: %02X:%02X:%02X Func: %s",
                    direction, transport, event->security_long_addr[0],
                    event->security_long_addr[1],
                    event->security_long_addr[2], event->function);

        case MOCHAD_EVENT_DETAIL_RFCAM:
            return format_checked(buffer, buffer_len, "%s %s %s",
                    direction, transport, event->camera_command);
    }
