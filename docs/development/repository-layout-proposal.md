...
The historical CGI set was withdrawn to `docs/research/legacy-cgi/` and removed
from distribution after a security review. The implemented tree therefore
intentionally differs from the proposed tree by archiving it as research rather
than moving it to `contrib/cgi/`. Empty proposed directories were not created.
