...
- **`MSG_NOSIGNAL` does not exist.** `platform.h` falls back to `0`, which
  keeps `send()` valid but does not by itself suppress `SIGPIPE`.
  `mochad_ignore_sigpipe()` (`src/net/socket_io.c`) covers this: it is called
  once from `mydaemon()`, before any listener exists, and ignores `SIGPIPE`
  process-wide rather than per socket via `SO_NOSIGPIPE`, because the daemon
  never `exec()`s (so nothing inherits the ignored disposition) and a
  per-socket call at a future accept() site could be forgotten.
