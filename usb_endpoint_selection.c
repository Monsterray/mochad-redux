...
int mochad_select_interrupt_endpoints(const struct libusb_config_descriptor *config,
    ...
    if (in_endpoint == NULL || out_endpoint == NULL || in_packet_size == NULL ||
    ...
    for (i = 0; i < config->bNumInterfaces; i++) {
        ...
        for (j = 0; j < iface->num_altsetting; j++) {
            ...
            for (k = 0; k < alt->bNumEndpoints; k++) {
                ...
                unsigned int packet_size = endpoint->wMaxPacketSize & 0x07ff;
                int is_interrupt = (endpoint->bmAttributes & LIBUSB_TRANSFER_TYPE_MASK) ==
