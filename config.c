...
static void set_error2(char *error, size_t error_len, const char *prefix,
        const char *value)
{
    if (error_len == 0)
        return;
...
static int copy_string(char *dest, size_t dest_len, const char *value,
...
static int parse_port(const char *name, const char *value, int *port,
        char *error, size_t error_len)
{
    char *endptr = NULL;
    long parsed;
    ...
    if (errno || endptr == value || *endptr != '\0' ||
        ...
        snprintf(error, error_len, "%s must be a TCP port from 1 to 65535: %s",
                name, value);
        return -1;
    }
...
static int parse_dual_stack(const char *value,
        mochad_dual_stack_policy *policy)
{
    char normalized[24];
    size_t i;
    size_t len;
...
static int parse_log_level(const char *value, int *level)
{
    ...
    parsed = strtol(value, &endptr, 10);
    if (!errno && endptr != value && *endptr == '\0' &&
            parsed >= LOG_EMERG && parsed <= LOG_DEBUG) {
        *level = (int)parsed;
        return 0;
    }
    ...
    if (strcmp(normalized, "emerg") == 0 ||
            strcmp(normalized, "emergency") == 0)
        *level = LOG_EMERG;
    else if (strcmp(normalized, "alert") == 0)
        *level = LOG_ALERT;
    else if (strcmp(normalized, "crit") == 0 ||
            strcmp(normalized, "critical") == 0)
        *level = LOG_CRIT;
    else if (strcmp(normalized, "err") == 0 ||
            strcmp(normalized, "error") == 0)
        *level = LOG_ERR;
    else if (strcmp(normalized, "warning") == 0 ||
            strcmp(normalized, "warn") == 0)
        *level = LOG_WARNING;
...
static int apply_key_value(mochad_config *config, const char *raw_key,
        const char *raw_value, char *error, size_t error_len)
{
    char key[64];
    char value[256];
    int bool_value;
    ...
    if (strlen(raw_value) >= sizeof(value)) {
        set_error2(error, error_len, "configuration value too long for key: ",
                raw_key);
        return -1;
    }
    ...
    if (strcmp(key, "bind") == 0 || strcmp(key, "bind_address") == 0)
        return copy_string(config->bind_address,
                sizeof(config->bind_address), value, error, error_len);

    if (strcmp(key, "port") == 0 || strcmp(key, "server_port") == 0 ||
            strcmp(key, "main_port") == 0)
        return parse_port(raw_key, value, &config->server_port,
                error, error_len);

    if (strcmp(key, "xml_port") == 0)
        return parse_port(raw_key, value, &config->xml_port,
                error, error_len);

    if (strcmp(key, "openremote_port") == 0)
        return parse_port(raw_key, value, &config->openremote_port,
                error, error_len);

    if (strcmp(key, "xml_enabled") == 0 ||
            strcmp(key, "enable_xml") == 0) {
    ...
    if (strcmp(key, "openremote_enabled") == 0 ||
            strcmp(key, "enable_openremote") == 0) {
        if (parse_bool(value, &bool_value) < 0) {
            set_error2(error, error_len, "invalid boolean for ", raw_key);
            return -1;
        }
    ...
    if (strcmp(key, "dual_stack") == 0) {
        if (parse_dual_stack(value, &config->dual_stack) < 0) {
            set_error2(error, error_len,
                    "dual_stack must be auto, enable, or disable: ", value);
            return -1;
        }
        return 0;
    }
...
static int load_config_file(mochad_config *config, const char *path,
        char *error, size_t error_len)
{
    FILE *file;
    char line[512];
    ...
    file = fopen(path, "r");
    if (file == NULL) {
        snprintf(error, error_len, "could not open config file %s: %s",
                path, strerror(errno));
        return -1;
    }
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (equals == NULL) {
            snprintf(error, error_len,
                    "invalid config file line %lu: expected key=value",
                    line_number);
            fclose(file);
            return -1;
        }
        ...
        *equals = '\0';
        ...
        if (apply_key_value(config, key, value, error, error_len) < 0) {
            ...
            snprintf(detail, sizeof(detail),
                    "invalid config file line %lu: %s", line_number, error);
...
static int apply_env_value(mochad_config *config, const char *env_name,
        const char *key, char *error, size_t error_len)
{
...
static int apply_environment(mochad_config *config,
        char *error, size_t error_len)
{
    if (apply_env_value(config, "MOCHAD_BIND", "bind", error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_BIND_ADDRESS", "bind",
                error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_PORT", "port", error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_SERVER_PORT", "port",
                error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_XML_ENABLED", "xml_enabled",
                error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_XML_PORT", "xml_port",
                error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_OPENREMOTE_ENABLED",
                "openremote_enabled", error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_OPENREMOTE_PORT",
                "openremote_port", error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_FOREGROUND", "foreground",
                error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_RAW_DATA", "raw_data",
                error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_DUAL_STACK", "dual_stack",
                error, error_len) < 0)
        return -1;
    if (apply_env_value(config, "MOCHAD_LOG_LEVEL", "log_level",
                error, error_len) < 0)
        return -1;
...
static int require_arg(int argc, char **argv, int *index,
        char *error, size_t error_len)
{
    if (*index + 1 >= argc) {
...
static int find_config_file_arg(int argc, char **argv,
        char *path, size_t path_len, char *error, size_t error_len)
{
    int i;
...
static int apply_cli(mochad_config *config, int argc, char **argv,
        char *error, size_t error_len)
{
    int i;
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--config") == 0) {
            ...
            if (require_arg(argc, argv, &i, error, error_len) < 0)
                return -1;
            if (apply_key_value(config, "xml_port", argv[i],
                        error, error_len) < 0)
                return -1;
        }
            ...
            if (require_arg(argc, argv, &i, error, error_len) < 0)
                return -1;
            if (apply_key_value(config, "openremote_port", argv[i],
                        error, error_len) < 0)
                return -1;
        }
            ...
            if (require_arg(argc, argv, &i, error, error_len) < 0)
                return -1;
            if (apply_key_value(config, "dual_stack", argv[i],
                        error, error_len) < 0)
                return -1;
        }
            ...
            if (require_arg(argc, argv, &i, error, error_len) < 0)
                return -1;
            if (apply_key_value(config, "log_level", argv[i],
                        error, error_len) < 0)
                return -1;
        }
        ...
        char *error, size_t error_len)
{
    ...
    if (find_config_file_arg(argc, argv, config_path, sizeof(config_path),
                error, error_len) < 0)
        return -1;

    if (config_path[0] != '\0') {
        if (copy_string(config->config_file, sizeof(config->config_file),
                    config_path, error, error_len) < 0)
            return -1;
        ...
        char *error, size_t error_len)
{
    ...
    if (rc != 0) {
        snprintf(error, error_len,
                "bind address must be a numeric IPv4 or IPv6 address: %s",
                config->bind_address);
        return -1;
    }
    freeaddrinfo(result);
    ...
    if ((config->xml_enabled && config->server_port == config->xml_port) ||
            (config->openremote_enabled &&
             config->server_port == config->openremote_port) ||
        ...
        const mochad_config *config)
{
    ...
    written = snprintf(buffer, buffer_len,
            ...
            mochad_config_log_level_name(config->log_level),
            config->server_port,
            config->xml_enabled ? "true" : "false",
            config->xml_port,
            config->openremote_enabled ? "true" : "false",
            config->openremote_port);
