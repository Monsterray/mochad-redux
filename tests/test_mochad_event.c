...
static int test_legacy_body_golden_outputs(void)
{
    ...
    if (expect_text(buffer, "Rx PL HouseUnit: A1",
                "PL selected house/unit output changed"))
        return 1;

    event.function = "On";
    if (mochad_event_format_legacy_body(&event, buffer, sizeof(buffer)) < 0)
        return expect(0, "PL house/unit event did not format");
    if (expect_text(buffer, "Rx PL HouseUnit: A1 Func: On",
                "PL house/unit output changed"))
        return 1;
    ...
    if (expect_text(buffer, "Rx RF HouseUnit: A1 Func: On",
                "RF house/unit output changed"))
        return 1;
    ...
    if (expect_text(buffer, "Tx RF House: B Func: Dim",
                "RF house output changed"))
        return 1;
    ...
    if (expect_text(buffer, "Rx PL House: C Func: Bright(7)",
                "PL dim/bright output changed"))
        return 1;
    ...
    if (expect_text(buffer, "Rx RFSEC Addr: 0xE2 Func: ARM_KR10A",
                "short RFSEC output changed"))
        return 1;
    ...
    if (expect_text(buffer,
                "Rx RFSEC Addr: 01:23:45 Func: CONTACT_ALERT_MIN",
    ...
    return expect_text(buffer, "Tx RFCAM A CAMUP",
            "RFCAM output changed");
}
...
static int test_native_and_xmlsocket_framing(void)
{
    ...
    length = mochad_event_format_openremote_line(&event, buffer,
            sizeof(buffer));
    if (length < 0)
    ...
    length = mochad_event_format_xmlsocket_line(&event, buffer,
            sizeof(buffer));
    if (length < 0)
        return expect(0, "XMLSocket line did not format");
    if (expect(memcmp(buffer, expected, strlen(expected)) == 0,
                "XMLSocket body changed"))
        return 1;
    if (expect(buffer[strlen(expected)] == '\0',
                "XMLSocket delimiter changed"))
        return 1;
    return expect((size_t)length == strlen(expected) + 1,
            "XMLSocket length changed");
}
