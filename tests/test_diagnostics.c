...
int main(void)
{
    ...
    if (print_json("hello", mochad_diag_json_hello(json, sizeof(json),
                    "mochad 0.1.18"), json))
        return 1;
    if (print_json("capabilities", mochad_diag_json_capabilities(json,
                    sizeof(json), config.raw_data), json))
        return 1;
    if (print_json("health", mochad_diag_json_health(json, sizeof(json),
                    "mochad 0.1.18", &runtime), json))
        return 1;
    if (print_json("clients", mochad_diag_json_clients(json, sizeof(json),
                    &runtime), json))
        return 1;
    if (print_json("config", mochad_diag_json_config(json, sizeof(json),
                    &config), json))
        return 1;
    if (print_json("version", mochad_diag_json_version(json, sizeof(json),
                    "mochad 0.1.18"), json))
        return 1;
