...
static int test_initial_submit_failure_does_not_leave_queue_busy(void)
{
    ...
    if (expect(x10_write(first, sizeof(first)) == -EIO,
                "initial write failure should propagate"))
        return 1;
    if (expect(PollTimeOut == -1,
                "initial write failure should leave polling idle"))
        return 1;
    ...
    if (expect(x10_write(second, sizeof(second)) == (int)sizeof(second),
        ...
        return 1;
    if (expect(last_write_len == sizeof(second) &&
                memcmp(last_write, second, sizeof(second)) == 0,
                "second write did not reach USB after first failure"))
        return 1;

    return expect(PollTimeOut == 2000,
            "successful write should arm queue timeout");
}
...
static int test_queued_submit_failure_does_not_advance_head(void)
{
    ...
    if (expect(x10_write(queued, sizeof(queued)) == (int)sizeof(queued),
                "busy write should queue"))
        return 1;

    calls_before = write_usb_calls;
    write_usb_result = -EBUSY;
    if (expect(send_next_x10out() == -EBUSY,
                "queued submit failure should propagate"))
        return 1;
    ...
    write_usb_result = 0;
    if (expect(send_next_x10out() == 0,
                "queued item should retry after submit failure"))
        return 1;
